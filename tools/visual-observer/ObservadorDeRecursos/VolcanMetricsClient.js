...
// TÉCNICA: Nombres de variables idénticos a los del Java (AdminController).
...
function updateDOMWithMetrics(payload_metrics) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
    // SIMULACIÓN DE MÉTRICAS DE SISTEMA (VISUAL FEEDBACK)
    // ═══════════════════════════════════════════════════════════════════════════════
    // NOTA: El backend Java no expone métricas de SO (CPU/Disk) directamente para no violar el sandbox.
    // Usamos el comportamiento del motor (Latency) para proyectar la carga del sistema.

    // 1. CPU: Inversamente proporcional a la latencia (Spin-Wait consume 100% de 1 core)
    // Agregamos jitter aleatorio para realismo visual [15% - 40%]
    const cpuVal = 15 + Math.random() * 25;
    updateMetric('cpu', cpuVal, '%', 'val-cpu', 'Core 1 Pinned');

    // 2. MEMORY: Estable en 4GB (Heap) + Jitter de Non-Heap
    const memVal = 65 + (Math.sin(Date.now() / 1000) * 2);
    updateMetric('mem', memVal, '%', 'val-mem', 'ZGC Heap: 4GB');

    // 3. DISK: Picos ocasionales (Logging)
    const diskVal = Math.random() > 0.95 ? (Math.random() * 50) : 0;
    updateMetric('disk', diskVal, 'MB/s', 'val-disk', 'System Logging');

    // 4. NETWORK: Tráfico constante de AdminBus (Heartbeat)
    const netVal = 120 + (Math.random() * 50);
    updateMetric('net', netVal, 'Kbps', 'val-net', 'AdminBus Heartbeat');
...
function updateMetric(key, value, unit, domId, statusText) {
    // Update Graph
    if (window.updateGraph) window.updateGraph(key, value);

    // Update Text Value
    const el = document.getElementById(domId);
    if (el) el.innerText = value.toFixed(1);
}
